...
코드 배치도 실제 데이터 인젝션 흐름에 맞춰
입력 준비 → 파싱 → 청킹 → 임베딩 → 벡터 DB 저장 → 검색 순서로 정리했습니다.
...
class RegulationRAG:
    ...
    #
    # 0. 입력 준비
    #    TXT 또는 향후 API 정규화 데이터를 준비합니다.
    #
    # 1. 파싱
    #    법령 원문에서 장·절·조를 찾고 조 전체 Parent를 만듭니다.
    #
    # 2. 청킹
    #    Parent 안의 항·호·목을 검색용 Child로 나눕니다.
    #
    # 3. 임베딩
    #    Child 텍스트를 multilingual-e5-base 벡터로 변환합니다.
    #
    # 4. 벡터 DB 저장
    #    Child·벡터·metadata를 ChromaDB에 저장하고
    #    Parent 조문은 별도 JSON 저장소에 보관합니다.
    #
    # 5. 검색
    #    질문을 임베딩하고 가까운 Child를 찾은 뒤 Parent를 복원합니다.
    #
    # 참고:
    # Python 클래스의 메서드는 정의된 순서와 관계없이 호출할 수 있습니다.
    # 아래 배치는 실행 조건이 아니라 사람이 처리 흐름을 따라가기 위한 순서입니다.
    # ========================================================
    ...
    def load_records(
        ...
        for item in normalized_records:
            ...
            # 1. 파싱:
            # 원문에서 장·절·조를 찾아 조 전체 Parent를 만듭니다.
            ...
            for parent in parents:
                ...
                # 2. 청킹:
                # Parent 안의 항·호·목을 검색용 Child로 만듭니다.
        ...
        for start in range(
            ...
            # 3. 임베딩:
            # 현재 batch의 Child 텍스트를 E5 벡터로 변환합니다.
            batch_embeddings = self._embed_documents(
                batch_documents
            )

            # 4. 벡터 DB 저장:
            # Child, 벡터, metadata를 같은 순서로 ChromaDB에 저장합니다.
        ...
        # Child와 parent_id로 연결되는 조 전체 Parent를 JSON에 저장합니다.
    ...
    # ========================================================
    # 5. 검색 - Query 임베딩, Child 검색, Parent 복원
    # ========================================================
